...
define(['N/log', 'N/search', 'N/record', '../common/f3_bm_constants', "/SuiteScripts/Modules/generaltoolsv1.js"], function (log, search, record, CONSTANTS_MODULE, GENERALTOOLS) {
    ...
    var searchPendingFulfillmentSO = function searchPendingFulfillmentSO(context, perPage) {
        try {
            var transactionSearchObj = search.create({
                ...
                filters: [['status', 'anyof', 'SalesOrd:B', 'SalesOrd:D'], 'AND', ['mainline', 'is', 'T']/*, 'AND', ['custbody_bkm_override_autofill', 'is', 'T']*/],
    ...
    var searchPendingCustomFulFillOrders = function searchPendingCustomFulFillOrders() {
        try {
            return search.create({
                ...
                filters: [
                    ["custrecord_f3_process_status", "anyof", "1", "3"],
                    "AND",
                    ["created","after","12/10/2021 11:59 pm"],
                    "AND",
                    ["custrecord_f3_sales_order_id.mainline","is","T"]
                ],
                columns: [search.createColumn({
                    name: 'id',
                    sort: search.Sort.ASC,
                    ...
                    join: "CUSTRECORD_F3_SALES_ORDER_ID",
                    label: "Sales Channel"
                }),
                    search.createColumn({
    ...
    var searchInventoryDetailsForItem = function searchInventoryDetailsForItem(item, location, join,saleschannel) {
        try {
            if (saleschannel=='4') {binn="3097";binn1="15197"} // Mercantile
            if (saleschannel=='2') {binn="7545";binn1="7545"} // eCommerce
            if (saleschannel=='1') {binn="7545";binn1="7545"} // QVC DS

            var d = new Date();
            var n = d.getHours();

            log.error({  title: "saleschannel: ", details: saleschannel});
            var itemSearchObj = search.create({
                ...
                filters: [['internalid', 'anyof', item], 'AND',
                    ...
                    [["".concat(join,".binnumber"),"anyof",[binn]],"OR",["".concat(join,".binnumber"),"anyof",[binn1]]]],
    ...
    var createItemFulfillment = function createItemFulfillment(customFulfillOrdersRecord) {
        ...
        var saleschannel = customFulfillOrdersRecord.values[CUSTOM_FULFILL_ORDERS.FIELDS.SALES_CHANNEL.ID].value;
        ...
        if (saleschannel==2) {
            ...
            log.error({  title: "varrut.backorder: ", details: varrut.backorder});
            if (varrut.backorder ) {
                values[CUSTOM_FULFILL_ORDERS.FIELDS.PROCESS_STATUS.ID] = CONSTANTS_MODULE.PROCESS_STATUS.PROCESSED_WITH_ERROR.ID;
                values[CUSTOM_FULFILL_ORDERS.FIELDS.ERROR.ID] = "Sale Order contains item (s) in backorder " + soId
        ...
        try {
            ...
            values[CUSTOM_FULFILL_ORDERS.FIELDS.PROCESS_STATUS.ID] = CONSTANTS_MODULE.PROCESS_STATUS.PROCESSED_WITH_ERROR.ID;
            values[CUSTOM_FULFILL_ORDERS.FIELDS.ERROR.ID] = "Error Name: " + String(e.name) + " | " + saleschannel + " Error Message: " + String(e.message)
            submitCustomFulfillOrdersRecordFields({
                type: customFulfillOrdersRecord.recordType,
                id: customFulfillOrdersRecord.id,
                values: values
        ...
        var _loop = function _loop(i) {
            ...
            if (inventoryDetailAvail === 'T') {
                ...
                if (!itemFulfillmentRecord.hasSublistSubrecord({
                    ...
                    var itemInventoryDetails = searchInventoryDetailsForItem(item, fulfillLocation, join,saleschannel);
                    ...
                    if (!itemSearchResultCount) {
                        ...
                        var values = {};
                        values[CUSTOM_FULFILL_ORDERS.FIELDS.PROCESS_STATUS.ID] = CONSTANTS_MODULE.PROCESS_STATUS.PROCESSED_WITH_ERROR.ID;
                        values[CUSTOM_FULFILL_ORDERS.FIELDS.ERROR.ID] = "Sale Order contains item (s) is not in binlocacion " + soId
                        submitCustomFulfillOrdersRecordFields({
                            type: customFulfillOrdersRecord.recordType,
                            id: customFulfillOrdersRecord.id,
                            values: values
