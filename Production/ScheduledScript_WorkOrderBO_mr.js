...
define(['N/log', 'N/search', 'N/record'],
    function (log, search, record) {
        ...
        var getInputData = function getInputData(context) {
            ...
            filters:
            [
                [[["formulatext:  case when {item.inventorylocation}='Kissimmee - Warehouse' then CASE WHEN NVL({item.locationquantityavailable}, 0)<({quantity}-NVL({quantitycommitted}, 0))   THEN 'YES'  ELSE 'NO' END END","contains","YES"],"AND",["type","anyof","WorkOrd"],"AND",["mainline","is","F"],"AND",["status","anyof","WorkOrd:B","WorkOrd:D","WorkOrd:A"]],"OR",[["custbody_totalitemsbo","greaterthan","0"],"AND",["type","anyof","WorkOrd"],"AND",["mainline","is","T"]]], 
                "AND", 
