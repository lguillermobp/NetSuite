...
define(["N/search", "N/file", "N/render", "N/runtime", "N/format", "N/xml", "N/log", "/SuiteScripts/Modules/LoDash.js"],
    ...
    function (search, file, render, runtime, format, xml,log,  _) {
        ...
        function onRequest(context) {
            ...
            let pdf = file.load({id: "/SuiteScripts/WorkOrderPDF.xml"}).getContents();
            ...
            const searchWorkOrderHeader = search.create({
                ...
                "columns": [
                    {
                        ...
                        "name": "custbody_bkmn_wo_cust_po_num",
                        "label": "Customer PO Number",
                        "type": "text",
                        "sortdir": "NONE"
                    }, {
                        "name": "custbody_bkmn_wo_so_req_ship_date",
                ...
                pdf = pdf.replace("[NEIGHBOR_PO]", result.getValue({name: "custbody_bkmn_wo_cust_po_num"}));
                pdf = pdf.replace("[SALES_ORDER_REQ_SHIP_DATE]", result.getValue({name: "custbody_bkmn_wo_so_req_ship_date",}));
            ...
            const searchWorkOrderLines = search.create({
                ...
                workOrderLines += `<td>${result.getValue({name: "salesdescription", join: "item"})}</td>`;
            ...
            for (const result of inventoryBalanceData) {
                ...
                inventoryBalanceLines += `<td>${result.inventorynumber}</td>`
                inventoryBalanceLines += `<td>${result.expirationdate}</td>`
                inventoryBalanceLines += `<td>${result.datecreated}</td>`
                inventoryBalanceLines += `<td>${result.onhand}</td>`
                inventoryBalanceLines += `<td>${result.available}</td>`
