...
define(["N/error", "N/search", "N/file", "/SuiteScripts/Modules/LoDash.js"],
    /**
     *
     * @param error
     * @param search
    ...
    function (error, search, file, _) {
        ...
        function getdocumentBackOrder() {
            ...
            let searchInventoryBalance = search.load({
                id: "customsearch_bks_ecomm_5_2"
            });

            let pagedData = searchInventoryBalance.runPaged({
                pageSize: 1000
            });
            ...
            pagedData.pageRanges.forEach(function (pageRange) {
                var page = pagedData.fetch({index: pageRange.index});
                page.data.forEach(function (result) {
                    let document = Number(result.getValue({ name: "tranid" }));
        ...
        function pivotSalesOrderData(salesOrderData,numberOfPriorityOrders,allowPartialShipping) {
            ...
            for (let i = 1; i < salesOrderData.length; i++) {
                ...
                if (!allowPartialShipping && salesOrderData[i]["backOrder"]=='1') { noprocesar='S'}
                if (noprocesar==='N') {
        ...
        function getTopNumberOfPrioritySalesOrdersAtSouth(numberOfPriorityOrders, salesOrderData,
            ...
            let inventoryBalance = getInventoryBalanceForSouth();
            let priorityOrders = [];
