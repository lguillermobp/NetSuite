            ...
            page.data.forEach(function (fresult) {
                ...
                if (billofm[billid]) {
                    ...
                    plantext = '^'+billofm[billid].assembly+'^,^'+descriptionassembly+'^,^'+billofm[billid].costassembly+'^,^'+billofm[billid].billofmaterialsid+'^,^'+billofm[billid].billofmaterials+'^,^'
                        ...
                        +fresult.getValue({name: "bomquantity",join: "component"})+'^,^'+fresult.getValue({name: "baseunits",join: "component"})+'^,^'+pageitems1[componentid].lastpurchasevendor+'^,^'+pageitems1[componentid].lastpurchasedate+'^&&';
    ...
    function findCases3() {
        ...
        pagedData.pageRanges.forEach(function (pageRange) {
            ...
            page.data.forEach(function (fresult3) {
                ...
                item=fresult3.getText({name: "item",summary: "GROUP"});
                internalid=fresult3.getValue( {name: "internalid", join: "vendor", summary: "GROUP"});
                entityid=fresult3.getValue( {name: "entityid", join: "vendor", summary: "GROUP"});
                itemtype=fresult3.getValue( {name: "itemtype",  summary: "GROUP"});
                trandate=fresult3.getValue( {name: "trandate",  summary: "MAX"});
